...
@inject(stores => ({
    ...
    addUser: (user, isFriend = false) => {
        if (isFriend) {
            stores.autoReply.addFriendUser(user);
        } else {
            stores.autoReply.addGroupUser(user);
        }
        stores.search.reset();
    },
    clear: (e) => {
...
export default class AutoReply extends Component {
    ...
    addUser(user, isFriend) {
        this.props.addUser(user, isFriend);
    ...
    navigation(e) {
        ...
        switch (e.keyCode) {
            ...
            case 13:
                ...
                this.addUser([...result.friend, ...result.groups, ...history, ...getPlaceholder()].find(e => e.UserName === active.dataset.userid), !helper.isChatRoom(active.dataset.userid));
        }
    }

    renderUser(user, isFriend) {
        return (
            <div className={classes.user} onClick={e => this.addUser(user, isFriend)} data-userid={user.UserName}>
            ...
            <div>
                ...
                </header>
                {
                    list.map((e, index) => {
                        return (
                            <div key={index}>
                                {this.renderUser(e, title.toLowerCase() === 'friend' ? true : (title.toLowerCase() === 'groups' ? false : !helper.isChatRoom(e.UserName)))}
                            </div>
                        );
                    })
                }
